...
from tcmagent.chunk import make_chunk_id, make_citation_markdown, make_citation_url
...
def validate_candidate_chunks(
    ...
    stats = build_stats(normalized_chunks, chunk_lengths, source_match_ratios)
...
def normalize_raw_chunk(
    ...
    if len(text) < MIN_CHUNK_CHARS:
        warnings.append(f"Chunk {index} is very short ({len(text)} chars).")
