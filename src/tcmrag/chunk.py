...
source document with hyperlinks such as:

    source://tcm_basic_theory#page=23
"""
...
DEFAULT_SOURCE_URL = "source://tcm_basic_theory"
...
def extract_pages(pdf_path: str | Path) -> list[dict[str, Any]]:
    ...
    empty_pages: list[dict[str, Any]] | None = None
    for extractor in (
        _extract_pages_with_pymupdf,
        _extract_pages_with_pypdf,
        _extract_pages_with_pdftotext,
    ):
        pages = extractor(pdf_path)
        if pages is None:
            continue
        if pages_have_text(pages):
            return pages
...
def pages_have_text(pages: list[dict[str, Any]]) -> bool:
    return any(str(page.get("text") or "").strip() for page in pages)
...
def make_citation_url(source_url: str, pdf_page: int) -> str:
    """Append a PDF page fragment to a citation base URL."""
